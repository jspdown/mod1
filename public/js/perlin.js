...
PerlinNoise = new function () {
    ...
    this.permutation;
    this.size;
    this.initNoise = function (map, size) {
        this.p = new Array(size * size * size * size);
        this.permutation = new Array(size * size);
        this.size = size;
        for (var j = 0; j < size; j++) {
            for (var i = 0; i < size; i++) {
                this.permutation[i] = map[j][i];
            }
        }
    }
    ...
    for (var i=0; i < this.size ; i++) {
        this.p[this.size+i] = this.p[i] = this.permutation[i];
    }

    var X = Math.floor(x) & this.size,                  // FIND UNIT CUBE THAT
        Y = Math.floor(y) & this.size,                  // CONTAINS POINT.
    ...
    return scale(lerp(w, lerp(v, lerp(u, grad(this.p[AA], x, y, z),  // AND ADD
